...
$stmt = $pdo->prepare('SELECT * FROM products WHERE subcat_id=7 ORDER BY name DESC LIMIT ?,?');
...
$total_products = $pdo->query('SELECT * FROM products WHERE subcat_id=7')->rowCount();
?>

<?=template_header('Pasta & Pasta Sauces')?>
...
<div class="products content-wrapper">
    ...
    <div class="buttons">
        ...
        <a href="index.php?page=pastaandsauce&p=<?=$current_page-1?>">Prev</a>
        <?php endif; ?>
        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
        <a href="index.php?page=pastaandsauce&p=<?=$current_page+1?>">Next</a>
        <?php endif; ?>
    </div>
</div>
