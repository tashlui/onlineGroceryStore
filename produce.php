...
$stmt = $pdo->prepare('SELECT * FROM products WHERE category_id=1 ORDER BY name DESC LIMIT ?,?');
...
$total_products = $pdo->query('SELECT * FROM products WHERE category_id=1')->rowCount();
?>

<?=template_header('Produce')?>
...
<div class="products content-wrapper">
    ...
    <div class="buttons">
        ...
        <a href="index.php?page=produce&p=<?=$current_page-1?>">Prev</a>
        <?php endif; ?>
        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
        <a href="index.php?page=produce&p=<?=$current_page+1?>">Next</a>
        <?php endif; ?>
    </div>
</div>
